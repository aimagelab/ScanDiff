...
from src.utils.diffusion_utils import discretized_gaussian_log_likelihood, normal_kl
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.sqrt_recipm1_alphas_cumprod = np.sqrt(1.0 / self.alphas_cumprod - 1) # see equation 15 from ddpm paper
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        Compute the mean and variance of the diffusion posterior:
            q(x_{t-1} | x_t, x_0)
        """
        assert x_start.shape == x_t.shape #here x_start is x0 from eq 15 ddpm paper
        posterior_mean = (
            _extract_into_tensor(self.posterior_mean_coef1, t, x_t.shape) * x_start #this is mean computation from eq 7 ddpm paper
            + _extract_into_tensor(self.posterior_mean_coef2, t, x_t.shape) * x_t
        )
        posterior_variance = _extract_into_tensor(self.posterior_variance, t, x_t.shape) #is the variance beta_tilde from eq 7
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if self.model_mean_type == ModelMeanType.START_X:
            ...
            pred_xstart = process_xstart( #this is x0 from eq 15 ddpm paper
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps): #eq 15 ddpm paper
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms['embedding_loss'] = self.embedding_loss(x_start, padding_mask, get_fixations, gt_scanpath)
    ...
    def reconstruction_loss(self, x_t, padding_mask, get_fixations, ground_truth_scanpath):
        ...
        reg_loss = (loss_fn_x(masked_reconstruction[:,:,0], masked_ground_truth[:,:,0]).sum(dim=1)/padding_mask.sum(dim=1)
                    +
                    loss_fn_y(masked_reconstruction[:,:,1], masked_ground_truth[:,:,1]).sum(dim=1)/padding_mask.sum(dim=1))
        
        t_loss = (loss_fn_t(masked_reconstruction[:,:,2], masked_ground_truth[:,:,2]).sum(dim=1)/padding_mask.sum(dim=1))
        
        return reg_loss, t_loss
    
    def embedding_loss(self, x_t, padding_mask, get_fixations, ground_truth_scanpath):
        reconstruction = get_fixations(x_t)
        loss_fn = th.nn.L1Loss(reduction='none')
        masked_reconstruction = reconstruction * padding_mask.unsqueeze(-1)
        masked_ground_truth = ground_truth_scanpath * padding_mask.unsqueeze(-1)
        
        loss = loss_fn(masked_reconstruction, masked_ground_truth)
        loss = loss.sum(dim=1) / padding_mask.unsqueeze(-1).sum(dim=1)
    ...
    def _get_x_start(self, x_start_mean, std):
        '''
        Scanpath embedding projection from {Emb(s)} to {x_0}, s represents the scanpath
        :param x_start_mean: scanpath embedding
        :return: x_0
        '''
    ...
    def token_validity_loss(self, sample_x, token_predictor, padding_mask):
        loss_fn = nn.CrossEntropyLoss(reduction='none')
        logits = token_predictor(sample_x)
        preds = logits.reshape(-1, 2)
        target = padding_mask.unsqueeze(-1).reshape(-1,1).squeeze()
        
        loss = loss_fn(preds, target).view(padding_mask.shape).mean(dim=-1)
