...
class PositionEmbeddingSine1d(nn.Module):
    def __init__(self, max_len, hidden_dim=512, temperature=10000, normalize=False, scale=None, device = "cuda:0"):
...
class PositionEmbeddingSine2d(nn.Module):
    def __init__(self, spatial_dim, hidden_dim=768, temperature=10000, normalize=False, scale=None, flatten = True, device = "cuda:0"):
        ...
        if flatten:
            self.pos =  self.pos.view(hidden_dim, -1).permute(1,0).unsqueeze(0)
        else:
